...
- `parent`中允许了定义一个与现有事实冲突的事实。这个问题检查起来也比较复杂：能想到的一种解决方式是在定义事实时检查是否能与现有事实unify（实际上就是做一次查询），但即便不能unify也分两种情况：一种是新定义的事实与现有事实冲突，另一种是新定义的事实与现有事实无关。要区分两者需要检查新定义的事实中包含的常量是否全部在知识库中存在，实现起来也比较复杂。
